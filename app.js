...
(function () {
  ...
  function renderActionCloud(root) {
    ...
    // The SVG's viewBox stays fixed at base size so all positions and
    // ring radii remain stable across zoom levels — zoom only scales
    // the rendered width/height (and the wrapper scrolls). Mirrors the
    // Open Points bubble-view zoom pattern.
    const SVG = $('#cloudSvg');
    ...
    const zoom = Math.max(0.4, Math.min(3, _cloudState.zoom || 1));
    const renderW = Math.round(W * zoom);
    const renderH = Math.round(H * zoom);
    ...
    const placed = allActions.map(({ a, proj: p }, i) => {
      ...
      if (_cloudState.preset === 'A') {
        // X: sym-log days-to-due, clamped to ±90 for layout (extremes still
        // shown but pinned to the edge). Items without a due date go in
        // their own lane at the far right past the +90-day mark.
        if (dtd == null) {
          xRaw = 7.5; // beyond +90's sym-log of ~6.5
          xMin = -6.6; xMax = 7.5;
        } else {
          xMin = -6.6; xMax = 7.5;
        ...
        // Preset B
        xMin = 0; xMax = Math.max(1, owners.length - 1);
        xRaw = ownerIdx.has(a.owner) ? ownerIdx.get(a.owner) : owners.length;
        if (xRaw === owners.length) { xMax = Math.max(1, owners.length); } // unassigned lane
        // Y: composite importance sym-log so the bottom band stays readable.
        const imp = compositeScores[i];
        yRaw = Math.log2(1 + imp);
        yMin = 0;
        yMax = Math.log2(1 + maxComp);
      }
      const x = padL + ((xRaw - xMin) / (xMax - xMin)) * innerW;
      const y = padT + (1 - (yRaw - yMin) / (yMax - yMin)) * innerH;
      return {
        id: a.id, a, proj: p, x, y, r, palette,
    ...
    // Light collision resolution — small repulsion preserving global
    // position so the scatter stays interpretable but bubbles don't
    // overlap. 30 iterations is plenty for ~250 nodes.
    (function relax() {
      ...
      // Clamp into viewport so over-pushed bubbles don't drift off-canvas.
      placed.forEach((p) => {
    ...
    // zoom. To keep axes / ticks / titles readable, we inject a
    // <style> block inside the SVG that re-declares those classes
    // with font-size and stroke-width multiplied by 1/zoom. The
    // CSS is scoped to this SVG only, so the rest of the app is
    // unaffected. Bubbles, heat rings, and bubble identifier labels
    // are NOT in this list — they're the plot content and should
    // scale with zoom.
    const iz = 1 / zoom;
    svgFrag += `<style>
      .cl-axis        { stroke-width: ${(1 * iz).toFixed(3)}px; }
      .cl-grid        { stroke-width: ${(0.5 * iz).toFixed(3)}px; }
      .cl-today       { stroke-width: ${(1.5 * iz).toFixed(3)}px; }
      .cl-tick        { font-size: ${(10.5 * iz).toFixed(2)}px; }
      .cl-today-lbl   { font-size: ${(10 * iz).toFixed(2)}px; }
      .cl-axis-title  { font-size: ${(11 * iz).toFixed(2)}px; }
      .cl-zone        { font-size: ${(11 * iz).toFixed(2)}px; stroke-width: ${(3 * iz).toFixed(3)}px; }
    </style>`;
    ...
    // ── Zoom controls ─────────────────────────────────────────────────
    // Setting zoom does not re-relax the layout; it just rescales the
    // rendered pixel size. The viewBox is constant, so all positions /
    // halos / labels stay correct. Wheel-zoom only when Ctrl/⌘ is held
    // so plain wheel still scrolls inside the canvas.
    const applyZoom = (z, anchor) => {
      const clamped = Math.max(0.4, Math.min(3, z));
      _cloudState.zoom = clamped;
      const sv = $('#cloudSvg');
      const sc = $('#cloudScroll');
      if (!sv || !sc) return;
      // Anchor zoom to a cursor point so zooming feels stable (the
      // point under the cursor stays under the cursor).
      const oldW = sv.clientWidth || sv.getBoundingClientRect().width;
      const oldH = sv.clientHeight || sv.getBoundingClientRect().height;
      const newW = Math.round(W * clamped);
      const newH = Math.round(H * clamped);
      sv.setAttribute('width', newW);
      sv.setAttribute('height', newH);
      // Patch the inline <style> block inside the SVG so axis labels +
      // grid + tick stroke widths re-scale to keep their rendered
      // pixel size constant across the new zoom level. The style block
      // was set at render time using a stale zoom factor; this update
      // is cheaper than a full re-render and keeps the axes readable
      // at every step.
      const izN = 1 / clamped;
      const styleEl = sv.querySelector('style');
      if (styleEl) {
        styleEl.textContent = `
          .cl-axis        { stroke-width: ${(1 * izN).toFixed(3)}px; }
          .cl-grid        { stroke-width: ${(0.5 * izN).toFixed(3)}px; }
          .cl-today       { stroke-width: ${(1.5 * izN).toFixed(3)}px; }
          .cl-tick        { font-size: ${(10.5 * izN).toFixed(2)}px; }
          .cl-today-lbl   { font-size: ${(10 * izN).toFixed(2)}px; }
          .cl-axis-title  { font-size: ${(11 * izN).toFixed(2)}px; }
          .cl-zone        { font-size: ${(11 * izN).toFixed(2)}px; stroke-width: ${(3 * izN).toFixed(3)}px; }
        `;
      }
