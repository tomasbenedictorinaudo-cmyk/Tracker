...
(function () {
  ...
  // Build the OOXML for one worksheet. Numbers go as <c t="n"><v>…</v>,
  // everything else as inline strings. Empty cells are omitted entirely
  // (xlsx supports sparse rows and that keeps the file small).
  ...
  function _buildProjectExportSheets(projects, multiProject) {
    ...
    // 1) Actions — register-style columns, plus milestone + deliverable names
    const actionRows = [];
    projects.forEach((p) => {
    ...
    sheets.push({
      ...
      headers: [
        ...
        'Owner', 'Status', 'Priority', 'Commitment %',
        'Start', 'Due', 'Predicted', 'Originator', 'Originator date',
        'Baseline due', 'Variance days', 'Logged hours', 'Planned hours',
        '% Complete', 'Tags', 'Depends on', 'Created', 'Updated',
      ],
      rows: actionRows,
    });
    ...
    // 8) KPIs — tall (long-form) snapshot so each row is one metric per
    // project. Easier to pivot than wide; also gracefully handles new
    // metrics over time. Includes action counts, delivery KPIs (adherence
    // + productivity over the last 30 days), and EVM rollup.
    const kpiRows = [];
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const todayStr = todayISO();
    const day = (n) => fmtISO(new Date(today.getTime() - n * dayMs));
    ...
    const computeProjectKpis = (p) => {
      const live = (p.actions || []).filter((a) => !a.deletedAt);
      const total = live.length;
      const done = live.filter((a) => a.status === 'done').length;
      const blocked = live.filter((a) => a.status === 'blocked').length;
      const late = live.filter((a) => a.due && !isClosedStatus(a.status) && dayDiff(a.due, todayStr) < 0).length;
      const upcoming = live.filter((a) => a.due && !isClosedStatus(a.status) && dayDiff(a.due, todayStr) >= 0 && dayDiff(a.due, todayStr) <= 7).length;
      const completion = total ? Math.round((done / total) * 100) : 0;
      const since14 = fmtISO(new Date(Date.now() - 14 * dayMs));
      const throughput14 = live.filter((a) => a.status === 'done' && a.updatedAt >= since14).length;
      // EVM rollup (closed-cancelled excluded — matches the dashboard tile).
      const evmScope = live.filter((a) => a.status !== 'cancelled');
      const ev = evmForActions(evmScope);
      // Adherence + productivity over the last 30 days — temporarily scope
      // deliveryMetricsForWindow to the single project by stashing
      // currentProjectId.
      const savedId = state.currentProjectId;
      state.currentProjectId = p.id;
      const delivery = deliveryMetricsForWindow(since30, todayStr);
      state.currentProjectId = savedId;
      return {
        total, done, blocked, late, upcoming, completion, throughput14,
        bac: ev.bac, pv: ev.pv, ev: ev.ev, ac: ev.ac,
        cpi: ev.cpi, spi: ev.spi, eac: ev.eac, etc: ev.etc,
        adherence: delivery.adherence,
        productivity: delivery.productivity,
        adherenceN: delivery.adherenceN,
        doneInWindow: delivery.doneInWindow,
        plannedHrs: delivery.plannedHrs,
        loggedHrs: delivery.loggedHrs,
      };
    };

    const push = (proj, metric, value, unit, notes) => kpiRows.push([
    ...
    projects.forEach((p) => {
      ...
      push(p, 'Total actions',           k.total,            'count', 'Live (non-deleted) actions');
      push(p, 'Done actions',            k.done,              'count');
      push(p, 'Open actions',            k.total - k.done,    'count');
      push(p, 'Late actions',            k.late,              'count', 'Open + past due');
      push(p, 'Blocked actions',         k.blocked,           'count');
      push(p, 'Due within 7 days',       k.upcoming,          'count', 'Open + due in next 7 days');
      push(p, 'Completion',              k.completion,        '%',     `${k.done} / ${k.total}`);
      push(p, 'Throughput (14d)',        k.throughput14,      'count', 'Actions marked done in the last 14 days');
      // Delivery KPIs (30-day window). Skip with note when sample too thin.
      if (k.adherenceN >= 5) {
        push(p, 'Schedule adherence (30d)', Number((k.adherence * 100).toFixed(1)), '%', `${k.adherenceN} due-by-now actions`);
      } else {
        push(p, 'Schedule adherence (30d)', '', '%', `Insufficient data (${k.adherenceN}/5 due-by-now actions)`);
      }
      if (k.doneInWindow >= 5) {
        push(p, 'Productivity (30d)', Number(k.productivity.toFixed(2)), '×', `${Math.round(k.plannedHrs)}h planned / ${Math.round(k.loggedHrs)}h logged across ${k.doneInWindow} done actions`);
      } else {
        push(p, 'Productivity (30d)', '', '×', `Insufficient data (${k.doneInWindow}/5 done-with-hours)`);
      }
      // EVM rollup. Round currencies; only emit indices when defined.
      if (k.bac > 0) {
        push(p, 'EVM — BAC', Math.round(k.bac), '€', 'Budget at completion across baselined actions');
        push(p, 'EVM — PV',  Math.round(k.pv),  '€', 'Planned value (work that should be done by today)');
        push(p, 'EVM — EV',  Math.round(k.ev),  '€', 'Earned value (work actually completed)');
        push(p, 'EVM — AC',  Math.round(k.ac),  '€', 'Actual cost (logged hours × rate)');
        if (k.cpi != null) push(p, 'EVM — CPI', Number(k.cpi.toFixed(3)), '×', 'EV ÷ AC. >1 = under budget');
        if (k.spi != null) push(p, 'EVM — SPI', Number(k.spi.toFixed(3)), '×', 'EV ÷ PV. >1 = ahead of schedule');
        if (k.eac != null) push(p, 'EVM — EAC', Math.round(k.eac), '€', 'BAC ÷ CPI');
        if (k.etc != null) push(p, 'EVM — ETC', Math.round(k.etc), '€', 'EAC − AC');
      } else {
        push(p, 'EVM rollup', '', '', 'No baselined actions yet — take a project baseline to populate');
      }
      // Reference counts so the snapshot is self-contained.
      push(p, 'Milestones',         (p.milestones    || []).length, 'count');
      push(p, 'Deliverables',       (p.deliverables  || []).length, 'count');
      push(p, 'Open points',        (p.openPoints    || []).length, 'count');
      push(p, 'Risks & Opps',       (p.risks         || []).length, 'count');
      push(p, 'Decisions',          (p.decisions     || []).length, 'count');
      push(p, 'Change requests',    (p.changes       || []).length, 'count');
    });
    sheets.push({
      name: 'KPIs',
      headers: [
