...
(function () {
  ...
  function renderActionCloud(root) {
    ...
    view.innerHTML = `
      <div class="page-head">
        <div>
          ...
          <div class="page-sub">2D scatter for rapid triage — click bubbles to select, toggle presets to verify both surface the same critical items. Done &amp; cancelled actions are excluded.</div>
        </div>
        <div class="page-actions">
    ...
    // ── Drag-select to zoom to a box ──────────────────────────────────
    // Mouse-down on the chart background (NOT on a bubble) starts a
    // drag; the user pulls a selection rectangle; on release the
    // viewport snaps to that rectangle. Releasing without dragging at
    // all (or releasing a too-small box) is treated as a no-op so
    // accidental clicks don't change the zoom.
    let dragState = null;
    SVG.addEventListener('mousedown', (e) => {
      if (e.button !== 0) return;
      if (e.target.closest('.cl-bubble-g')) return; // bubble click → selection
      // Only inside chart area
      const rect = SVG.getBoundingClientRect();
      const px = e.clientX - rect.x;
      const py = e.clientY - rect.y;
      if (px < padL || px > padL + innerW || py < padT || py > padT + innerH) return;
      dragState = { startX: px, startY: py, curX: px, curY: py };
      e.preventDefault();
    });
    SVG.addEventListener('mousemove', (e) => {
      if (!dragState) return;
      ...
      // Live-render the selection rect.
    ...
    SVG.addEventListener('mouseup', () => {
      ...
      dragState = null;
      if (dx < 8 || dy < 8) return; // too small — treat as click on background
      // Convert pixel box to viewport fractions.
      // x in [padL, padL+innerW] maps to xMinFrac..xMaxFrac
      const vp = _cloudState.viewport;
      const xMinNew = vp.xMinFrac + ((x1 - padL) / innerW) * (vp.xMaxFrac - vp.xMinFrac);
      const xMaxNew = vp.xMinFrac + ((x2 - padL) / innerW) * (vp.xMaxFrac - vp.xMinFrac);
      // y is inverted: top of chart = yMax
      const yMaxNew = vp.yMinFrac + (1 - (y1 - padT) / innerH) * (vp.yMaxFrac - vp.yMinFrac);
      const yMinNew = vp.yMinFrac + (1 - (y2 - padT) / innerH) * (vp.yMaxFrac - vp.yMinFrac);
      const MIN_SPAN = 0.03;
    ...
    // Cancel drag on leave/escape so a stuck drag-rect doesn't linger.
