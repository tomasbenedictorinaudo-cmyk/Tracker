...
(function () {
  ...
  // Repaint the bottom-right pill. Cheap; called on every saveState() and
  // on a 30-s tick so the "X min ago" stays fresh even when nothing else
  // changes.
  function refreshStorageStatusLine() {
    const el = $('#storageStatus');
    if (!el) return;
    const dot = $('#storageStatusDot');
...
${(!data.next.milestones.length && !data.next.deliverables.length && !data.next.actions.length) ? '<p class="empty">Nothing scheduled in the next 14 days.</p>' : ''}
  ...
  function init() {
    ...
    render();
    // Safety prompt first (most important on a fresh install), tour
    // second. Both no-op if their respective `*Seen` flag is already set.
