  ...
  function renderActionCloud(root) {
    ...
    $('#cloudCSV').addEventListener('click', () => {
      exportCSV(`action-cloud-${(curProjectIsMerged() ? 'all' : curProject().name).replace(/\s+/g, '_')}-${todayISO()}.csv`,
        allActions, [
          { header: 'Project',         value: ({ proj }) => proj.name },
          { header: 'Identifier',      value: ({ a }) => a.identifier || '' },
          ...
          { header: 'Component',       value: ({ a, proj }) => findComponent(proj, a.component)?.name || '' },
          { header: 'Due',             value: ({ a }) => a.due || '' },
          { header: 'Days to due',     value: ({ a }) => actionDaysToDue(a) ?? '' },
          { header: '% Complete',      value: ({ a }) => percentComplete(a) },
          { header: 'Composite imp.',  value: ({ a }) => actionImportance(a, ctx).toFixed(2) },
          { header: 'Selected',        value: ({ a }) => _cloudState.selection.has(a.id) ? 'yes' : '' },
        ]);
    });
  }
  /* ─────────────────────── /Action Cloud ─────────────────────────── */
