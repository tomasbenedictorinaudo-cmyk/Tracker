...
(function () {
  ...
  const _cloudState = {
    ...
    focusMode: false,
    focusN: 12,
    sizeBy: 'auto',              // 'auto' (preset-driven) or fixed override later
  };
  ...
  function renderActionCloud(root) {
    ...
    $('#cloudStat').textContent = `${allActions.length} action${allActions.length === 1 ? '' : 's'} · ${_cloudState.preset === 'A' ? 'Urgency × Progress' : 'Owner × Importance'}${_cloudState.focusMode ? ` · top ${_cloudState.focusN} highlighted` : ''}`;
