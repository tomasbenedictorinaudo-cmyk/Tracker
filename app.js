...
(function () {
  ...
  function requestBackupDir() {
    ...
    try {
      ...
      toast('Could not open folder picker: ' + (e?.message || e));
    ...
    return pickerPromise.then(async (h) => {
      ...
      toast('Folder picker error: ' + (e?.message || e));
    ...
    panel.innerHTML = `
      ...
      <div class="ab-body">
        ${supported ? '' : `<div class="ab-warn">
          Folder auto-backup needs the File System Access API, which is only
          available in Chromium browsers (Chrome, Edge, Brave, Arc). In this
          browser, use <strong>Download now</strong> below to save a backup
        ...
        <div class="ab-note ab-muted" style="font-size: 11px; line-height: 1.45; padding-top: 4px;">
          The folder keeps just two files at any time:
          <code>cockpit-latest.json</code> (this save) and
          <code>cockpit-previous.json</code> (the one before).
          Older backups are removed on each rotation.
        </div>
